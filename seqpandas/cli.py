...
@click.option('--samples-as-dict', is_flag=True, default=True, help='Parse sample columns as dictionaries')
...
def read_vcf_cmd(input_file, ignore_header, samples_as_dict, output):
    ...
    try:
        # Read the VCF file
        df = read_vcf(input_file, ignore_header=ignore_header, samples_as_dict_dtype=samples_as_dict)
...
@main.command()
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--format', '-f', type=click.Choice(['fasta', 'sam', 'vcf', 'bed']), required=True, help='File format')
